...
def login():
    ...
    # Check if 2FA is required for this user or based on policy
    totp_svc = getattr(current_app, "totp_service", None)
    settings = store.get_settings()

    # Determine if this is a remote request
    require_2fa_remote = settings and settings.require_2fa_for_remote
    is_remote = False
    ...
    if require_2fa_remote and is_remote and not user.totp_enabled:
        if audit:
            audit.log_event(
                "LOGIN_BLOCKED",
                user=username,
