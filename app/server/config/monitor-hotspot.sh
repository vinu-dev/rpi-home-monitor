...
led_setup_mode() {
    if [ -x "$LEDCTL" ]; then
        "$LEDCTL" setup --role server --force >/dev/null 2>&1 || true
        return
    fi
...
led_connected() {
    if [ -x "$LEDCTL" ]; then
        "$LEDCTL" healthy --role server --force --clear-activation >/dev/null 2>&1 || true
        return
    fi
...
led_off() {
    if [ -x "$LEDCTL" ]; then
        "$LEDCTL" off --role server --force >/dev/null 2>&1 || true
        return
    fi
    led_write trigger none
...
led_connecting() {
    if [ -x "$LEDCTL" ]; then
        "$LEDCTL" connecting --role server --force >/dev/null 2>&1 || true
        return
    fi
    led_write trigger timer
