...
### Added — networking + discovery (closes #90)
- **mDNS publish readiness** ([#198](https://github.com/vinu-dev/rpi-home-monitor/issues/198) → PR [#228](https://github.com/vinu-dev/rpi-home-monitor/pull/228)). `DiscoveryService.start()` now verifies the avahi-publish helper is still alive ~500 ms after spawn. avahi-publish-* exits immediately when avahi-daemon isn't on the bus, when a name collides, or when the TXT record is rejected — the previous implementation logged "advertisement started" while in reality nothing reached the wire. Failed publications now log the actual stderr and let the supervisor retry.
- **mDNS goodbye on hostname change** ([#200](https://github.com/vinu-dev/rpi-home-monitor/issues/200) → PR [#230](https://github.com/vinu-dev/rpi-home-monitor/pull/230)) and **raw-UDP goodbye fallback** ([#233](https://github.com/vinu-dev/rpi-home-monitor/issues/233) → PR [#234](https://github.com/vinu-dev/rpi-home-monitor/pull/234)). `wifi.set_hostname()` previously did `systemctl restart avahi-daemon`, which kills the daemon before it can broadcast RFC 6762 §10.1 cache-flush records — caches around the LAN kept the old name resolvable for the full TTL window. Now hands the name change to `avahi-set-host-name` (D-Bus) when allowed; falls through to a privilege-free raw-UDP cache-flush packet (TTL=0, cache-flush bit set, sent twice ~1 s apart per RFC 6762 §10) when the daemon refuses (camera user lacks D-Bus permission on the default image policy). Live-validated with `tcpdump`: two cache-flush packets captured, hex confirms `0x8001` CLASS field.
- **Boot-time hostname-resolution retry with backoff** ([#199](https://github.com/vinu-dev/rpi-home-monitor/issues/199) → PR [#235](https://github.com/vinu-dev/rpi-home-monitor/pull/235)). `lifecycle._resolve_server` was a one-shot `gethostbyname` at boot — if mDNS hadn't published yet (Avahi cold-start, multicast rate-limit, slow DHCP), the camera spent its early life logging warnings while heartbeats failed silently. Replaced with a daemon-thread retry: exponential backoff (2 s → 60 s, doubling), 5-minute deadline, interruptible-sleep stop event so shutdown joins within 5 s. On deadline expiry raises a structured `mdns_resolution_failed` hardware-fault on the heartbeat — operators see "Server name didn't resolve" instead of "camera offline".
- **IP-based fallback path: QR on setup completion** ([#201](https://github.com/vinu-dev/rpi-home-monitor/issues/201) → PR [#285](https://github.com/vinu-dev/rpi-home-monitor/pull/285)). Setup wizard's connecting view now renders a QR for `https://<ip>:443` alongside the textual `.local` URL. Vendored pure-JS QR generator (~3 KB, MIT) — no new Python dependency. On networks where multicast is blocked (a structural mDNS limitation, not something the camera can fix on its end), users scan the QR with their phone and reach the camera by IP.

### Fixed — config sync + boot reliability
- **`config_sync` stuck-pending loop broken** ([#231](https://github.com/vinu-dev/rpi-home-monitor/issues/231) → PR [#232](https://github.com/vinu-dev/rpi-home-monitor/pull/232)). Bidirectional config sync (ADR-0015) had a state-machine dead end: once `config_sync` flipped to `pending`, the only path back to `synced` was an explicit `/api/v1/cameras/config-notify` POST from the camera, which only fires for `origin="local"` applies in `status_server`. Server-origin retries (the very loop the heartbeat-driven `pending_config` is meant to deliver) never produced an ack, and the heartbeat reconciler was gated by `not had_pending`. Result: the camera applied the config every 15 s forever while the dashboard badge stayed ⌛ pending. Fix is defence-in-depth: server-side reconciler now treats a heartbeat-reported `stream_config` matching stored values as the implicit ack; camera-side `_apply_pending_config` fires `notify_config_change` regardless of origin. Either half alone breaks the loop on already-deployed cameras without re-flashing.
- **Throttle-state probe no longer hangs boot on quirky SoC sysfs** ([#289](https://github.com/vinu-dev/rpi-home-monitor/pull/289)). The #242 throttle work used `glob.glob("/sys/devices/platform/soc/**/throttled", recursive=True)`, which DOES follow symlinks per Python 3.12 docs. On the Pi Zero 2W's bcm2835 serial driver, `<dev>/driver/<dev>/driver/...` is a self-referential chain in sysfs — the probe spun in a CPU-bound loop forever, hanging `camera-streamer.service` at boot at "Config loaded" before any thread started. Replaced with `os.walk(soc_root, followlinks=False)` filtered by filename, capped at 2000 directories. Caught and fixed live during the 1.5.0 hardware test.

### Added — security + auth
- **TOTP-based 2FA for admin and remote users** ([#238](https://github.com/vinu-dev/rpi-home-monitor/issues/238)). RFC 6238 TOTP with QR enrollment, recovery codes, and a `pyotp>=2.9` runtime dependency declared in `app/server/setup.py`. Admin can enforce 2FA per-user; recovery codes are single-use and bcrypt-hashed at rest.
- **Active sessions UI + revocation** ([#246](https://github.com/vinu-dev/rpi-home-monitor/issues/246) → PR [#269](https://github.com/vinu-dev/rpi-home-monitor/pull/269)). New Settings → Sessions tab lists active server sessions with last-seen IP / user-agent / created timestamp. Admin can revoke individual sessions or "all but this one" with a single click.
- **Shareable view-only links for clips and live cameras** ([#244](https://github.com/vinu-dev/rpi-home-monitor/issues/244) → PR [#264](https://github.com/vinu-dev/rpi-home-monitor/pull/264)). Time-limited, revocable, no account required on the recipient side. Backed by `share_link_service.py` with audit-logged generation/revocation events.

### Added — reliability + observability
- **RPi throttling / under-voltage detection** ([#242](https://github.com/vinu-dev/rpi-home-monitor/issues/242) → PR [#265](https://github.com/vinu-dev/rpi-home-monitor/pull/265)). Camera heartbeats now carry the full `throttle_state` block (under-voltage / frequency-capped / throttled / soft-temp-limit, both now and sticky-since-boot). Read via `vcgencmd` first (preferred — fast, accurate); falls back to a sysfs probe with the symlink-loop fix above. Server stores the most recent sample on `cameras.json` and surfaces a dashboard badge for any sticky bit.
- **Clock-drift / NTP-sync health on dashboard** ([#250](https://github.com/vinu-dev/rpi-home-monitor/issues/250) → PR [#272](https://github.com/vinu-dev/rpi-home-monitor/pull/272)). Server-side `time_health_service` polls `timedatectl status`, surfaces NTP-synced + offset on the System Status tile. Camera heartbeats can now request a server-side `systemd-timesyncd` restart when their own clock drifts beyond a threshold.
- **systemd watchdog (sd_notify WATCHDOG=1)** ([#249](https://github.com/vinu-dev/rpi-home-monitor/issues/249) → PR [#271](https://github.com/vinu-dev/rpi-home-monitor/pull/271)). Both `monitor.service` and `camera-streamer.service` now ping the systemd watchdog from their main loops. systemd auto-restarts a hung process instead of the previous "active but doing nothing" dead state.
- **Motion clip pre-roll backend (phase 1)** ([#160](https://github.com/vinu-dev/rpi-home-monitor/issues/160) → PR [#284](https://github.com/vinu-dev/rpi-home-monitor/pull/284)). The first slice of the H.264 ring-buffer plan from the motion-detection design doc — backend plumbing only, kill switch defaults off. Phases 2–4 land in subsequent releases per the exec plan.

### Added — backup + audit + interop
- **Config backup and restore** ([#240](https://github.com/vinu-dev/rpi-home-monitor/issues/240)). Settings → Maintenance → Export config produces a portable archive of `cameras.json` + `users.json` + relevant `settings.json` keys (secrets stripped). Import flow rolls back on validation failure.
- **Offsite / cloud backup of recordings** ([#243](https://github.com/vinu-dev/rpi-home-monitor/issues/243) → PR [#263](https://github.com/vinu-dev/rpi-home-monitor/pull/263)). S3-compatible / rclone-based worker that uploads new clips out-of-band for tamper- and theft-resistance. Worker enforces a queue-cap to defend against an unreachable bucket starving local storage.
- **Export motion clips with embedded wall-clock timestamps** ([#251](https://github.com/vinu-dev/rpi-home-monitor/issues/251) → PR [#275](https://github.com/vinu-dev/rpi-home-monitor/pull/275)). Forensic-grade audit trail — exported clips carry burned-in wall-clock per frame so a third party can't dispute when an event happened. New `clip_stamper.py` + `clip_stamp_queue.py` services on the server.
- **Encoder parameter presets per camera** ([#252](https://github.com/vinu-dev/rpi-home-monitor/issues/252) → PR [#276](https://github.com/vinu-dev/rpi-home-monitor/pull/276)). Bitrate / codec / resolution profiles selectable from a dropdown instead of editing each field by hand. New `encoder_presets.py` service.
- **System diagnostics export** ([#253](https://github.com/vinu-dev/rpi-home-monitor/issues/253) → PR [#278](https://github.com/vinu-dev/rpi-home-monitor/pull/278)). Settings → Maintenance → Diagnostics produces a single tarball with logs, config, `cameras.json`, environment metadata — for handoff to support / forensics. Secrets stripped pre-export.
- **Audit log CSV/JSON export** ([#247](https://github.com/vinu-dev/rpi-home-monitor/issues/247) → PR [#270](https://github.com/vinu-dev/rpi-home-monitor/pull/270)). Audit log retains everything in JSON-line format anyway; this UI exposes a clean export endpoint for retention archives.
- **Quiet hours / scheduled silencing** ([#245](https://github.com/vinu-dev/rpi-home-monitor/issues/245) → PR [#267](https://github.com/vinu-dev/rpi-home-monitor/pull/267)). Per-user motion + system-notification mute windows (e.g. 22:00–07:00 every day). Notifications still hit the local Alert Center / audit log; just the toast/sound is suppressed.

### Changed — architecture
- **Split camera human-admin surface from machine control surface** ([#113](https://github.com/vinu-dev/rpi-home-monitor/issues/113) → PR [#282](https://github.com/vinu-dev/rpi-home-monitor/pull/282)). New `camera_streamer.control_server` module isolates the machine-to-machine RTSPS / control endpoints from the human-facing status page. Cleaner mTLS contract, easier security posture analysis. Touches `lifecycle.py`, `heartbeat.py`, and the systemd unit's port allocations.

### Internal
- New helpers + services on the camera side: `control_server.py`, `sd_notify.py`, `watchdog_notifier.py`, vendored `static/qrcode.min.js`.
- New services on the server side: `auth_totp.py`, `healthz.py`, `sessions.py`, `share.py`, `webhooks.py`, `timestamp_backfill.py`; helpers `backup_paths.py`, `camera_trust.py`, `clip_stamp_queue.py`, `clip_stamper.py`, `config_backup_service.py`, `diagnostics_bundle.py`, `encoder_presets.py`, `network_info.py`, `notification_schedule.py`, `offsite_backup.py`, `request_origin.py`, `session_service.py`, `share_link_service.py`, `throttle_state.py`.
- New runtime dependencies declared in `app/server/setup.py`: `pyotp>=2.9`, `boto3>=1.34`. Requires `pip install` on the deployed image (or a fresh Yocto rebuild that picks up the recipe changes).

### Documentation
- **Secrets-at-rest inventory + guardrail close-out** (tracks [#101](https://github.com/vinu-dev/rpi-home-monitor/issues/101)). Added [`docs/operations/secrets-inventory.md`](docs/operations/secrets-inventory.md) to document the currently-known persisted secrets and their residual-risk posture, added a pre-commit guard (`tools/secrets/check_persisted_secrets.py`) that refuses silent additions of new persisted secret fields without an inventory row or allowlist justification, and documented the `SECRET_KEY_ROTATED`, `TAILSCALE_AUTH_KEY_ROTATED`, and `CAMERA_PAIRING_SECRET_ROTATED` audit events. The opt-in LUKS migration track in [`docs/exec-plans/luks-post-pair-migration.md`](docs/exec-plans/luks-post-pair-migration.md) is unchanged by this PR.
- **`docs/guides/hardware-setup.md` — Pi Zero 2W WiFi band corrected** ([#197](https://github.com/vinu-dev/rpi-home-monitor/pull/197), closes [#184](https://github.com/vinu-dev/rpi-home-monitor/issues/184)). Three places claimed or implied 5GHz support; the Zero 2W ships 802.11 b/g/n (2.4GHz only). Replaced with the actually-actionable equivalent (move other 2.4GHz devices to 5GHz to free airtime, pick a less-crowded channel).
- **Motion-mode pre-roll exec plan** ([#202](https://github.com/vinu-dev/rpi-home-monitor/pull/202), tracks [#160](https://github.com/vinu-dev/rpi-home-monitor/issues/160)). Translates the original `docs/archive/exec-plans/motion-detection.md` §D5 decision (3-second H.264 ring buffer with `MOTION_PREROLL_ENABLED` kill switch) into four reviewable implementation phases. Phase 1 lands in this release.
- **Recording-scheduler module docstring corrected** ([#203](https://github.com/vinu-dev/rpi-home-monitor/pull/203)). Stale claim that motion mode was a no-op; corrected to match ADR-0021 reality.
- **Documentation reorganized** for human + AI agent navigation (`docs/doc-map.yml`, folder wayfinding indexes; supersedes the flat `docs/specs` and `docs/adr` paths used in earlier releases — those now live under `docs/history/`).

### Hardware verification
- All five mDNS sub-fixes (#198/#199/#200/#201/#233) verified live on the running Pi Zero 2W camera against the Pi 4B server before tag.
- #231 stuck-pending loop verified live: `cam-0fcea5cf` (which had been stuck on ⌛ pending in production) flipped to ✓ synced within 20 s of the server-side reconciler restart.
- #289 throttle-probe symlink-loop hang found and fixed during the 1.5.0 hardware test itself; cannot regress without test failures.
...
Tiny patch release. No code change in the camera or server image — the canonical 1.4.1 unit was already correct. This release adds a CI guard that prevents the class of systemd-hardening regression that stranded three cameras on 1.3.0 from re-shipping, and a matching AI execution rule. Same partition layout, same A/B flow.
