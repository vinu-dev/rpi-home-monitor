...
`docs/history/baseline/architecture.md`; this file assigns traceability IDs.

## System Architecture Items

| ID | Item | Description | Requirements | Risks | Security controls | Tests | Source/modules |
|---|---|---|---|---|---|---|---|
| ARCH-001 | Local monitoring topology | Cameras push video and events to the home server on the local network; browsers access the server dashboard over HTTPS. | SYS-001, SYS-002, SYS-004 | RISK-001, RISK-002 | SC-001, SC-002, SC-004 | TC-001, TC-006 | `docs/history/baseline/architecture.md` |
...
| ARCH-003 | Data and recording boundary | Recordings, config, certificates, logs, OTA state, and staged diagnostics bundles live under `/data` or configured external storage. | SYS-002, SYS-003, SYS-010, SYS-034 | RISK-003, RISK-006, RISK-026 | SC-005, SC-008, SC-025 | TC-003, TC-017, TC-055 | `docs/history/baseline/architecture.md`, `diagnostics_bundle.py` |
| ARCH-004 | Update boundary | Server and camera updates flow through SWUpdate bundles, staging/library storage, verification, camera-owned activation, version confirmation, and A/B rollback design. | SYS-009 | RISK-004 | SC-003 | TC-009, TC-013, TC-036 | ADR-0008, ADR-0014, ADR-0020 |
...
| ARCH-008 | Account administration boundary | User lifecycle, password policy, roles, active session state/revocation, and audit evidence form a privileged operator-management boundary. | SYS-014, SYS-004, SYS-010 | RISK-011, RISK-002 | SC-001, SC-011, SC-008 | TC-022, TC-011 | `user_service.py`, `session_service.py`, `auth.py`, `api/sessions.py` |
| ARCH-009 | Storage and media operation boundary | Recording storage, USB media, live media serving, deletion, and retention stay within canonical recording roots and selected devices, with root-side USB metadata and host-visible mounts. | SYS-016, SYS-026, SYS-003 | RISK-003, RISK-013, RISK-014 | SC-013, SC-014, SC-005 | TC-024, TC-025, TC-026, TC-027 | `usb.py`, `storage_service.py`, `recordings_service.py`, `live.py` |
...
| ARCH-015 | Operator evidence boundary | Logs, faults, audit records, timestamps, release versions, system summary records, and diagnostics bundles form operator/maintainer evidence surfaces. | SYS-020, SYS-022, SYS-023, SYS-034 | RISK-020, RISK-012, RISK-026 | SC-020, SC-008, SC-018, SC-025 | TC-017, TC-029, TC-041, TC-046, TC-055 | `audit.py`, `faults.py`, `system_summary_service.py`, `diagnostics_bundle.py` |
...
| Session administration | Operator lists active browser sessions and revokes one session or all other sessions. | Authenticated browser to server-side session store. | ARCH-008, SWR-001, SWR-009, SC-001 |
| USB and media operations | Operator scans, selects, formats, reconnects, ejects, deletes, or serves recording media. | Admin/session to filesystem, root-helper USB boundary, and removable media. | ARCH-009, SWR-027, SWR-028, SWR-029, SWR-030 |
| Diagnostics export | Admin requests a staged troubleshooting bundle that is assembled from local evidence surfaces, streamed back over HTTPS, and removed after download. | Authenticated admin to `/data` evidence staging and bounded subprocess collection. | ARCH-003, ARCH-015, SWR-068, SWR-069, SWR-070, SC-025 |
