...
| SWA-006 | OTA services | Bundle staging, reusable common camera bundle library, one-shot per-camera custom bundle jobs, verification, camera-owned activation reboot, target-version confirmation, update status, and install command boundary. | SWR-010, SWR-016, SWR-038 | RISK-004 | SC-003, SC-005 | TC-009, TC-013, TC-036 | `ota_service.py`, `camera_ota_client.py`, `ota_agent.py`, `ota_installer.py` |
...
| SWA-015 | Removable storage operations | USB detection, root-side filesystem metadata, host-visible mount, format, eject, selected target state, and safe fallback are centralized. Root-only mount/format/eject execution is isolated behind the monitor privileged-helper socket so the Flask web process can stay unprivileged. | SWR-027, SWR-028 | RISK-013, RISK-003 | SC-013, SC-005 | TC-024, TC-025 | `usb.py`, `storage_service.py`, `privileged.py`, `privileged_helper.py`, `monitor-privileged-helper.service` |
...
| SWA-022 | Camera setup/platform/health/status | Camera runtime validates config, WiFi setup, platform probes, health, local status surfaces, and shared LED state transitions. | SWR-035, SWR-036, SWR-037, SWR-042 | RISK-010, RISK-012, RISK-022 | SC-010, SC-012 | TC-033, TC-034, TC-035, TC-039 | `config.py`, `wifi.py`, `status_server.py`, `platform.py`, `health.py`, `status_led.py` |
...
| SWA-029 | Maintenance restart orchestration | Server and camera use a shared restart-schedule payload model with one scheduler per appliance, admin-gated immediate restart routes, OTA-busy guards, once-per-minute execution state, and latest-updated-at convergence between server-pushed control and camera-local configuration. | SWR-072 | RISK-001, RISK-015 | SC-001, SC-002, SC-008 | TC-057 | `restart_schedule_model.py`, `restart_schedule_service.py`, `restart_scheduler.py`, `camera_service.py`, `status_server.py`, `settings.html` |
| SWA-101 | Secrets-at-rest control triad | The operator-facing inventory page, the persisted-secret pre-commit guard, and the diagnostics-backed runtime smoke check share one canonical field vocabulary so contributors, reviewers, and operators see the same secrets-at-rest contract. | SWR-101-A, SWR-101-B, SWR-101-C | RISK-101-1, RISK-101-3 | SC-005, SC-101 | TC-101-AC-1, TC-101-AC-2, TC-101-AC-3, TC-101-AC-10, TC-101-AC-12, TC-101-AC-14 | `docs/operations/secrets-inventory.md`, `tools/secrets/check_persisted_secrets.py`, `scripts/smoke-test.sh`, `app/server/monitor/services/settings_service.py` |
